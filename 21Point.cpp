...
char CheckInput(char ch);
/*  This function is to do error detection.
It got user input and returns a right input. */
...
int main(){
	...
	do {
		...
		check_card = CheckInput(check_card);
		while(check_card == 'y' || check_card == 'Y'){
			...
			if(user_point > MAX_POINT) {
				cout<<"\nYou lose";
				cout<<"\nPlay again?";
				check_game = CheckInput(check_game);
				break;
			}
			cout<<"\nNeed one more card?(y/n):  ";
			check_card = CheckInput(check_card);
...
char CheckInput(char ch){
	do {
		...
		catch(runtime_error wrongInput){
